...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        .then(() => axiosGet('get-total-saved/').then(res => setTotalSaved(res.data)))
    ...
    return (
        <Box>
            ...
            <SimpleGrid columns={[1, 2, 2, 2, 2, 4]} spacing={6} mb={6}>
                ...
                <StatisticTile color={color} number={totalSaved} sentence="Saved overall" icon="piggy-bank" />
                <StatisticTile color={color} number={(goal - totalSaved).toFixed(2)} sentence="left until you've reached your goal" icon="coins" />
            </SimpleGrid>
            <SimpleGrid columns={[1, 1, 1, 1, 3, 3]} spacing={6}>
                <GoalProgress total_saved={totalSaved} goal={goal} color={color} />
